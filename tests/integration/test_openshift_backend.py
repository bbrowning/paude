...
def cleanup_test_resources(test_namespace: str, unique_session_name: str):
    ...
    sandbox_name = f"paude-{unique_session_name}"
...
class TestOpenShiftSessionLifecycle:
    ...
    def test_create_session_creates_sandbox_and_pvc(
        self,
        ...
        """Creating a session creates Sandbox and PVC."""
        ...
        assert resource_exists(
            "sandbox.agents.x-k8s.io", sandbox_name, test_namespace
        )
    ...
    def test_delete_session_removes_resources(
        ...
        """Deleting a session removes Sandbox, PVC, and NetworkPolicy."""
        config = SessionConfig(
            name=unique_session_name,
            workspace=temp_workspace,
            image=kubernetes_test_image,
...
class TestOpenShiftSandboxSpec:
    """Test Sandbox specification generated by the backend."""

    def test_sandbox_has_correct_labels(
        self,
        require_kubernetes: None,
        openshift_backend: OpenShiftBackend,
        test_namespace: str,
        ...
        """Sandbox has correct labels for session identification."""
        config = SessionConfig(
            name=unique_session_name,
            workspace=temp_workspace,
            image=kubernetes_test_image,
    ...
    def test_sandbox_has_pvc_template(
        self,
        require_kubernetes: None,
        openshift_backend: OpenShiftBackend,
        test_namespace: str,
        ...
        """Sandbox has volumeClaimTemplate for workspace PVC."""
        config = SessionConfig(
            name=unique_session_name,
            workspace=temp_workspace,
            image=kubernetes_test_image,
        ...
        sandbox = json.loads(result.stdout)
        vct = sandbox.get("spec", {}).get("volumeClaimTemplates", [])
...
class TestOpenShiftScaling:
    """Test session start/stop via Sandbox scaling."""
    ...
    def test_stop_session_scales_to_zero(
        ...
        """Stopping a session scales Sandbox to 0 replicas."""
        config = SessionConfig(
            name=unique_session_name,
            workspace=temp_workspace,
            image=kubernetes_test_image,
